...
CAudioSourceDecoder::ProcessDataInternal(CParameter & Parameters)
{
    ...
    try {
        ...
        flog::error("DRM SuperFrame parse EXCEPTION {}", e.what());
        bGoodValues = false;
    }
...
CAudioSourceDecoder::InitInternal(CParameter & Parameters)
{
    ...
    */
    ...
    audiodecoder = "";
        ...
        flog::info("Using audio decoder: {}", audiodecoder);
        ...
        flog::debug("DecOpen sample rate {}", inputSampleRate);
        ...
        if (numFrames == 0) {
            ...
            flog::debug("samplesPerChannelPerSuperFrame={} numFrames={}", samplesPerChannelPerSuperFrame, numFrames);
        }
        flog::debug("iLenDecOutPerChan={}", iLenDecOutPerChan);
        ...
        flog::debug("iMaxLenResamplerOutput={}", iMaxLenResamplerOutput);

        if (inputSampleRate != outputSampleRate) {
            if (iLenDecOutPerChan == 0)
                flog::debug("DRM iLenDecOutPerChan == 0: numFrames={} iLenDecOutPerChan={} inputSampleRate={} outputSampleRate={}",
